...
public class witch_Ai2 : entity
{
    ...
    public override void Update()
    {
        ...
        if (randompattern&&queuedspellindex<0&&move_duration<=0)
        {
            ...
            else
            {
                ...
                queuedspellindex = 0; // Clear queued spell index
    ...
    {
        if(queuedspellindex < 0) return;
        Debug.Log($"Witch Ai2 SpawnMagic called for {name} with queuedspellindex {queuedspellindex}");
        if(randompattern)
        {
        ...
        if( queuedspellindex >= magicspawns.Count)return;
